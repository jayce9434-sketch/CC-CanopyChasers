...
## v5 chat + Morse pass
- Lobby chat is text-only input; bots can still use optional device text-to-speech voices.
- Bot dialogue now prioritizes the actual sentence meaning, emotional context, plans, follow-up yes/no answers, named game systems, and action commands before lore/fallback replies.
- Morse remains a 6.66% fresh Dark Caves roll and now changes targets heavily toward bots, uses kick/ban/ghost/invisible/sound/stalk actions more often, and can trigger lobby panic/escapes.
- Toggleable MORSE FORM is in the thumb mod menu: black 000 body, long arms, Morse beeps/scary sounds, and bot panic reactions. Existing Ghost/Invisible/Kick/Ban tools can be combined with it.

## MORSE FORM V6
- `BE MORSE` now transforms the player into the actual black `000` Morse look instead of a mostly-normal player skin.
- Morse Form uses much longer arms, white pupil-less eyes, Morse markings, screen/static pulses, frequent Morse beeps and creepy sound cues.
- Bots now recognize the transformed player as Morse continuously, stop following, flee from the player, scream in chat, and may escape to another map.
- Talking while transformed gets fear-aware replies instead of lines like “how did you become Morse?”.
- Morse Form periodically phases/flickers like a ghost so the transformation feels active even when no other mod is enabled.
- The real cave Morse was also made more active: it favors bot targets more heavily and uses kick, ban, ghost, invisible, stalking and sound actions more frequently.
