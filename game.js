...
const defaults={
  ...
  socialMemory:{facts:{},events:[],recent:[]}
};
let save={...defaults,stats:{...defaults.stats},equipped:{...defaults.equipped},socialMemory:{facts:{},events:[],recent:[]}};
try{
  const s=JSON.parse(localStorage.getItem('canopyChaseSave')||'null');
  if(s){
    save={...defaults,...s,stats:{...defaults.stats,...(s.stats||{})},equipped:{...defaults.equipped,...(s.equipped||{})},socialMemory:{facts:{...(s.socialMemory?.facts||{})},events:Array.isArray(s.socialMemory?.events)?s.socialMemory.events.slice(-28):[],recent:Array.isArray(s.socialMemory?.recent)?s.socialMemory.recent.slice(-18):[]}};
...
function ensureSocialMemory(){
  if(!save.socialMemory||typeof save.socialMemory!=='object')save.socialMemory={facts:{},events:[],recent:[]};
  if(!save.socialMemory.facts||typeof save.socialMemory.facts!=='object')save.socialMemory.facts={};
  if(!Array.isArray(save.socialMemory.events))save.socialMemory.events=[];
  if(!Array.isArray(save.socialMemory.recent))save.socialMemory.recent=[];
}
ensureSocialMemory();
...
let platforms=[],bots=[],roundTimer=0,activeGlowFraction=0,morse=null,morseRollDone=false,playerMorseNextSound=0,playerMorseNextPanic=0,playerMorseNextPhase=0,playerMorseNextAbility=0,playerMorseInvisibleUntil=0,playerMorseGhostUntil=0,playerMorseShell=null,playerMorseFlash=0;
...
function newBot(id,respawn=false){
  ...
  const b={
    ...
    memory:{playerName:save.playerName,lastTopic:'lobby',previousTopic:'',lastPlayerLine:'',lastPromise:'',lastBotLine:'',lastEntity:null,lastEvent:null,lastUserMood:null,metAt:time,history:[],events:[],facts:{}}
  };
  return b;
}
function fillBots(){
...
function handWorld(hand){let x=hand.x,y=hand.y;if(player.mods.morseForm){const m=Math.hypot(x,y)||1,min=108;if(m<min){x=x/m*min;y=y/m*min}}return{x:player.x+x,y:player.y+y};}
...
function botSpeak(bot,text,voice=false){
  ...
  if(bot.memory){
    bot.memory.lastBotLine=full;
    if(!Array.isArray(bot.memory.history))bot.memory.history=[];
    bot.memory.history.push({who:'bot',text:full,t:now()});
    if(bot.memory.history.length>18)bot.memory.history.splice(0,bot.memory.history.length-18);
  }
...
function memoryEventLabel(ev){
  if(!ev)return'';
  const labels={
    morseBan:'Morse banned you',
    morseKick:'Morse kicked you',
    morseSeen:'you saw Morse',
    morseFear:'Morse scared you',
    morseLeavePlan:'you said you would leave if Morse appeared',
    tagged:'you got tagged',
    mapLike:`you said you like ${ev.map&&MAPS[ev.map]?MAPS[ev.map].name.toLowerCase():'that map'}`,
    mapDislike:`you said you do not like ${ev.map&&MAPS[ev.map]?MAPS[ev.map].name.toLowerCase():'that map'}`
  };
  return labels[ev.type]||ev.summary||'that';
}
function parsePlayerExperience(text,bot=null){
  const s=cleanSpeech(text),mem=bot?.memory||{},lastEntity=mem.lastEntity||save.socialMemory?.facts?.lastEntity||'';
  const byMorse=/\bmorse\b/.test(s)||(/\b(he|him|his)\b/.test(s)&&lastEntity==='morse');
  if((/\bi (?:got|was|have been|ve been|been) banned\b/.test(s)&&byMorse)||/\bmorse (?:actually )?banned me\b/.test(s)||/\bwhen morse banned me\b/.test(s))
    return{type:'morseBan',entity:'morse',summary:'Morse banned you',raw:text,t:now()};
  if((/\bi (?:got|was|have been|ve been|been) kicked\b/.test(s)&&byMorse)||/\bmorse (?:actually )?kicked me\b/.test(s)||/\bwhen morse kicked me\b/.test(s))
    return{type:'morseKick',entity:'morse',summary:'Morse kicked you',raw:text,t:now()};
  if((/\bi (?:saw|seen|found|ran into|met) morse\b/.test(s))||(/\bmorse\b/.test(s)&&/\bwas (?:right )?(?:there|behind me|next to me)\b/.test(s)))
    return{type:'morseSeen',entity:'morse',summary:'you saw Morse',raw:text,t:now()};
  if((/\b(scared|afraid|terrified|freaked|spooked|creeped)\b/.test(s)&&byMorse)||/\bmorse scared me\b/.test(s))
    return{type:'morseFear',entity:'morse',summary:'Morse scared you',raw:text,t:now()};
  if(/\b(if|when)\b.*\bmorse\b.*\b(join|spawn|appear|show up|come)\b/.test(s)&&/\b(i'?m|i am|i'?ll|i will|im gonna|i am gonna|ima|imma)\b.*\b(leave|run|dip|get out|go)\b/.test(s))
    return{type:'morseLeavePlan',entity:'morse',summary:'you said you would leave if Morse appeared',raw:text,t:now()};
  if(/\bi (?:got|was|have been|ve been) tagged\b|\bthey tagged me\b/.test(s))
    return{type:'tagged',entity:'tag',summary:'you got tagged',raw:text,t:now()};
  const liked=/\bi (?:really )?(?:like|love) (?:the )?(forest|cave|dark caves|plaza|hat plaza|leaderboards?)\b/.exec(s);
  if(liked){
    const map=mapMention(liked[1]);if(map)return{type:'mapLike',entity:map,map,summary:`you like ${MAPS[map].name}`,raw:text,t:now()};
  }
  const disliked=/\bi (?:really )?(?:hate|dislike|dont like|don't like) (?:the )?(forest|cave|dark caves|plaza|hat plaza|leaderboards?)\b/.exec(s);
  if(disliked){
    const map=mapMention(disliked[1]);if(map)return{type:'mapDislike',entity:map,map,summary:`you do not like ${MAPS[map].name}`,raw:text,t:now()};
  }
  return null;
}
function sharedEventCount(type){
  ensureSocialMemory();
  return save.socialMemory.events.filter(e=>e&&e.type===type).length;
}
function rememberSharedPlayerLine(text,intent,event=null){
  ensureSocialMemory();
  const rec={text:String(text).slice(0,160),intent,t:now(),entity:event?.entity||null};
  pushLimited(save.socialMemory.recent,rec,18);
  if(event){
    pushLimited(save.socialMemory.events,{...event,raw:String(event.raw||text).slice(0,160)},28);
    if(event.entity)save.socialMemory.facts.lastEntity=event.entity;
    save.socialMemory.facts.lastEventType=event.type;
    save.socialMemory.facts.lastEventAt=event.t||now();
    if(event.type==='morseBan')save.socialMemory.facts.morseBanCount=(save.socialMemory.facts.morseBanCount||0)+1;
    if(event.type==='morseKick')save.socialMemory.facts.morseKickCount=(save.socialMemory.facts.morseKickCount||0)+1;
    if(event.type==='morseSeen')save.socialMemory.facts.hasSeenMorse=true;
    if(event.type==='morseFear')save.socialMemory.facts.afraidOfMorse=true;
    if(event.type==='mapLike'&&event.map)save.socialMemory.facts.lastLikedMap=event.map;
    if(event.type==='mapDislike'&&event.map)save.socialMemory.facts.lastDislikedMap=event.map;
  }
  try{localStorage.setItem('canopyChaseSave',JSON.stringify(save));}catch(e){}
}
function rememberForBot(bot,text,intent,event=null){
  if(!bot?.memory)return;
  const mem=bot.memory;
  if(!Array.isArray(mem.history))mem.history=[];
  if(!Array.isArray(mem.events))mem.events=[];
  if(!mem.facts||typeof mem.facts!=='object')mem.facts={};
  pushLimited(mem.history,{who:'player',text:String(text).slice(0,160),intent,t:now()},18);
  mem.previousTopic=mem.lastTopic;mem.lastTopic=intent;mem.lastPlayerLine=text;mem.playerName=save.playerName;
  const a=analyzeUtterance(text,bot);if(a.topic)mem.lastEntity=a.topic;
  if(event){
    pushLimited(mem.events,{...event,raw:String(event.raw||text).slice(0,160)},14);
    mem.lastEvent=event.type;
    if(event.entity)mem.lastEntity=event.entity;
    if(event.type==='morseBan')mem.facts.morseBanCount=(mem.facts.morseBanCount||0)+1;
    if(event.type==='morseKick')mem.facts.morseKickCount=(mem.facts.morseKickCount||0)+1;
    if(event.type==='morseSeen')mem.facts.hasSeenMorse=true;
    if(event.type==='morseFear')mem.facts.afraidOfMorse=true;
    if(event.type==='mapLike'&&event.map)mem.facts.lastLikedMap=event.map;
    if(event.type==='mapDislike'&&event.map)mem.facts.lastDislikedMap=event.map;
  }
}
function latestRelevantEvent(bot){
  const own=Array.isArray(bot?.memory?.events)?bot.memory.events:[];
  if(own.length)return own[own.length-1];
  ensureSocialMemory();
  return save.socialMemory.events.length?save.socialMemory.events[save.socialMemory.events.length-1]:null;
}
function hasBotMemory(bot,type){
  return Array.isArray(bot?.memory?.events)&&bot.memory.events.some(e=>e&&e.type===type);
}
function memoryAwareReply(bot,text,intent){
  const s=cleanSpeech(text),mem=bot.memory||{},v=bot.personality||'friendly';
  const ev=parsePlayerExperience(text,bot);
  const latest=latestRelevantEvent(bot);
  const asksRemember=/\b(do you remember|you remember|remember when|remember that|did i tell you|i told you)\b/.test(s);

  if(ev?.type==='morseBan'){
    const prior=hasBotMemory(bot,'morseBan')||sharedEventCount('morseBan')>1;
    bot.knowsMorse=true;
    if(asksRemember){
      return prior
        ? pick(['YES 😭 I remember. You told me Morse ban-gunned you and you got stuck solo for two minutes.','I remember that 😭 Morse banned you and the offline timer had to count down before multiplayer came back.','yeah, I remember you telling me that. Morse hit you with the ban gun — nasty.'])
        : 'I dont remember hearing that before, but I do now 😭 Morse actually banned you?';
    }
    if(prior)return pick(['yeah, I remember that 😭 Morse banned you before. I would still be mad about it.','I remember you telling me. Getting randomly ban-gunned by Morse is actually evil 😭','RIGHT, the time Morse banned you 😭 I remember. At least that timer keeps counting while youre offline.']);
    return pick(['WAIT Morse actually banned you before?? 😭 thats brutal.','Morse BAN-GUNNED you?? nahhh 😭 I would be so annoyed.','you got the full Morse ban screen?? 😭 okay yeah, thats a story I would remember.']);
  }
  if(ev?.type==='morseKick'){
    const prior=hasBotMemory(bot,'morseKick')||sharedEventCount('morseKick')>1;bot.knowsMorse=true;
    if(asksRemember&&prior)return pick(['yeah 😭 I remember you saying Morse kicked you.','I remember. Morse randomly booted you and you had to rejoin.']);
    return prior?pick(['yeah, I remember that kick 😭 Morse really chose violence.','you told me about that 😭 getting randomly kicked by Morse is so rude.']):pick(['Morse actually kicked you?? 😭','BRO 😭 he really just booted you out of the room?']);
  }
  if(ev?.type==='morseSeen'){
    const prior=hasBotMemory(bot,'morseSeen')||sharedEventCount('morseSeen')>1;bot.knowsMorse=true;
    return prior?pick(['yeah, I remember you saw him before 😭 black 000 and those long arms.','I remember. You already ran into Morse once — I would be checking every corner after that.']):pick(['you actually SAW Morse before?? 😭','okay so youve actually run into him. that explains why you keep talking about Morse 😭']);
  }
  if(ev?.type==='morseFear'){
    bot.knowsMorse=true;
    if(hasBotMemory(bot,'morseBan'))return pick(['I get why 😭 after Morse literally banned you, I would be scared when those beeps start too.','yeah, after that ban-gun thing? completely fair. I would not trust Morse either.']);
    if(hasBotMemory(bot,'morseKick'))return pick(['yeah, after Morse kicked you before, I get why hearing him again would freak you out.','fair 😭 once he has already kicked you, every beep would make me nervous too.']);
    return pick(['yeah, I get it. Morse is way worse when youve actually had him come after you.','I believe you 😭 the disappearing plus the sounds would get me too.']);
  }
  if(ev?.type==='morseLeavePlan'){
    bot.knowsMorse=true;mem.facts.leaveIfMorse=true;
    return pick(['honestly same 😭 and I remember that — if Morse appears, youre OUT.','fair. If I hear the beeps after you say that, im running with you 😭','got it: Morse joins, you leave. I am probably following you out too.']);
  }
  if(ev?.type==='tagged')return hasBotMemory(bot,'tagged')?pick(['again?? 😭 you keep getting caught at the worst time.','I remember you got tagged before too 😭 we need a better escape route.']):pick(['NOOO 😭 getting tagged always happens at the worst time.','dang 😭 next round stay near me and we can juke together.']);
  if(ev?.type==='mapLike'&&ev.map)return pick([`okay, im remembering that — you like ${MAPS[ev.map].name.toLowerCase()}.`,`W. ${MAPS[ev.map].name} is going in my memory as one of your maps 😭`]);
  if(ev?.type==='mapDislike'&&ev.map)return pick([`got it 😭 you do NOT like ${MAPS[ev.map].name.toLowerCase()}.`,`okay, im remembering that. I wont act surprised if you wanna leave ${MAPS[ev.map].name.toLowerCase()}.`]);

  if(asksRemember){
    if(latest){
      const label=memoryEventLabel(latest);
      return pick([`yeah, I remember — ${label}.`,`I remember that. ${label}.`,`yep. ${label}. I didnt forget 😭`]);
    }
    return'I dont have an older story from you in my memory yet. Tell me once and I will remember it.';
  }

  if(latest&&/^(it|that|he|him|morse)\b/.test(s)){
    if(/\b(scary|creepy|terrifying|freaky|bad|awful|crazy)\b/.test(s)){
      if(latest.type==='morseBan')return pick(['yeah 😭 getting ban-gunned out of nowhere would scare me too.','I get it. The ban screen popping up because Morse picked YOU is creepy.']);
      if(latest.type==='morseKick')return pick(['yeah, getting randomly kicked by him would make every beep feel suspicious 😭','I get why. Once Morse has already kicked you, you know he can actually mess with the lobby.']);
      if(latest.type==='morseSeen'||latest.type==='morseFear')return pick(['yeah. seeing Morse for real is way different from just hearing people talk about him 😭','I get it. Black 000, long arms, disappearing — that would freak me out too.']);
    }
    if(/\b(funny|hilarious|cool|awesome)\b/.test(s)){
      if(latest.type==='morseBan')return pick(['okay 😭 getting banned is awful but I can see why the randomness is funny after its over.','fair 😭 it is kind of hilarious that Morse can just choose somebody and ruin their next two minutes.']);
      if(latest.type==='morseKick')return pick(['LOL yeah, once youre back in the room the random kick is kinda funny 😭','I would laugh AFTER I rejoined, not while it happened 😭']);
    }
    if(/\b(hated|hate|didnt like|don't like|annoying|mad)\b/.test(s)){
      if(latest.type==='morseBan')return pick(['I dont blame you 😭 two minutes locked out of multiplayer because Morse picked you is annoying.','yeah, I would hate that too. The offline timer helps, but still 😭']);
      if(latest.type==='morseKick')return pick(['fair 😭 getting booted randomly is annoying even if you rejoin fast.','yeah, I would be mad too if Morse just kicked me for no reason.']);
    }
  }

  if(/^(and then|then|after that|so yeah|yeah|exactly|right|fr|for real)$/.test(s)&&latest){
    if(latest.type==='morseBan')return pick(['yeah 😭 that Morse ban story is stuck in my head now.','right 😭 I remember the ban. I would avoid the cave for a minute after that.']);
    if(latest.type==='morseKick')return pick(['yeah 😭 I remember the kick. Morse really just ejected you.','right. That random Morse kick was foul 😭']);
  }
  return null;
}
...
function botActionFromPlayer(bot,text,intent){
  ...
  if(intent==='plan'){
    const liked=bot.memory?.facts?.lastLikedMap||save.socialMemory?.facts?.lastLikedMap;
    if(liked&&MAPS[liked]&&liked!==current&&chance(.72)){
      sendBotToMap(bot,liked,'remembered your favorite');
      return{type:'travel',map:liked,line:`you said you like ${MAPS[liked].name.toLowerCase()}, so lets go there 😭`};
    }
    return chooseBotPlan(bot,false);
  }
  return null;
}
...
function conversationalReply(bot,text,intent){
  ...
  const memoryReply=memoryAwareReply(bot,text,intent);if(memoryReply)return memoryReply;
  ...
  if(/\b(i got|i was|ive been|i've been) (kicked|banned|tagged)\b/.test(s)){
    if(/banned/.test(s))return pick(['NOOO 😭 getting banned is brutal.','thats rough 😭 if it was Morse, I would remember that one.','nahhh 😭 at least the timer keeps counting offline.']);
...
function responseFor(bot,text,intent,action=null){
  ...
  if(player.mods.morseForm&&botMap(bot)===save.map&&!player.mods.invisible){
    bot.knowsMorse=true;bot.morseSeen=true;bot.followPlayer=false;bot.followUntil=0;bot.fear=Math.max(bot.fear,2);bot.fleePlayerUntil=time+6;bot.state='fleePlayer';
    if(action?.type==='follow'||/follow me|come with me|stay with me/.test(s))return pick(['ABSOLUTELY NOT 😭 YOU ARE MORSE','NOPE IM RUNNING THE OTHER WAY','WHY WOULD I FOLLOW MORSE?! RUN']);
    if(intent==='greet'||/\b(hi|hello|hey|yo)\b/.test(s))return pick(['MORSE JUST SAID HI TO ME 😭 RUN','NOPE. MORSE IS TALKING NOW','WHY IS MORSE TALKING TO ME?!']);
    if(/dont be scared|not scary|calm down|its me|i am morse|im morse/.test(s))return pick(['THATS EXACTLY WHY IM SCARED 😭','YOU ARE LITERALLY 000 MORSE, NO','NOPE. BLACK BODY, LONG ARMS, BEEPS. IM RUNNING']);
    if(/where.*go|come here|wait|stop/.test(s))return pick(['IM GOING AWAY FROM YOU 😭','NOPE IM NOT STOPPING NEAR MORSE','ANYWHERE THAT ISNT NEXT TO MORSE']);
    return pick(['MORSE IS TALKING TO ME 😭','NOPE NOPE NOPE GET AWAY FROM ME','WHY IS 000 TALKING?! RUN','DONT COME CLOSER MORSE','IM NOT STAYING HERE WITH MORSE']);
  }
  ...
  const words=meaningfulWords(text).slice(0,4);
  if(words.length){
    const key=words[0];
    return v==='chaotic'?pick([`BRO 😭 ${key} is crazy`,`okay wait 😭 I get what youre saying about ${key}`]):
           v==='cautious'?pick([`yeah, I hear you about ${key}.`,`okay, I get what you mean about ${key}.`]):
           pick([`yeah, I get what you mean about ${key}.`,`okay, I hear you. ${key} makes sense in context.`]);
  }
  return pick(['yeah, I hear you.','okay, I get what you mean.','fair 😭']);
}
...
function playerSpeak(text){
  ...
  const intent=classify(text),event=parsePlayerExperience(text),responders=respondersForText(text);
  rememberSharedPlayerLine(text,intent,event);
  if(!responders.length){addChatMessage('SYSTEM','No bots are currently in this map.','system');return;}
  // Everyone in the current map reads public chat, even if only one or two bots answer.
  const responderSet=new Set(responders);
  botsHere().forEach(b=>{if(!responderSet.has(b))rememberForBot(b,text,intent,event);});
  const usedReplies=[];
  responders.forEach((b,i)=>setTimeout(()=>{
    ...
    usedReplies.push(reply);botSpeak(b,reply,i===0||intent==='morseReport');rememberForBot(b,text,intent,event);
...
function botSocialEvent(bot){
  const here=botMap(bot);
  if(player.mods.morseForm&&here===save.map&&!player.mods.invisible){
    bot.knowsMorse=true;bot.fear=Math.max(bot.fear,1.8);bot.fleePlayerUntil=time+3;bot.state='fleePlayer';
    return pick(['MORSE IS HERE RUN 😭','WHY IS 000 STILL FOLLOWING US','DONT STOP, MORSE IS RIGHT THERE','NOPE IM NOT STAYING NEAR MORSE','GET AWAY FROM THE BLACK 000 MONKE','I HEAR THE MORSE BEEPS AGAIN 😭']);
  }
  if(morse&&here==='cave'&&bot.knowsMorse){
...
$$('[data-mod]').forEach(b=>b.addEventListener('click',()=>{
  ...
  if(m==='morseForm'){
    playerMorseNextSound=time+.05;playerMorseNextPanic=time+.04;playerMorseNextPhase=time+rand(2.2,3.8);playerMorseNextAbility=time+rand(1.4,2.4);playerMorseInvisibleUntil=0;playerMorseGhostUntil=0;playerMorseShell=null;playerMorseFlash=1;
    if(player.mods.morseForm){
      toast('MORSE FORM • 000 • ON');morseBeep();scarySound('drone',true);setTimeout(()=>scarySound('static',true),180);setTimeout(()=>morseBeep(),430);
      addChatMessage('SYSTEM','000 // MORSE SIGNATURE DETECTED','system');
      addChatMessage('Morse','... --- ...','bot');
      botsHere().forEach((bot,i)=>{
        bot.knowsMorse=true;bot.morseSeen=true;bot.followPlayer=false;bot.followUntil=0;bot.fear=Math.max(bot.fear,2.45);bot.fleePlayerUntil=time+9999;bot.state='fleePlayer';bot.morseEscapeAt=time+rand(1.2,3.8);
        setTimeout(()=>{if(!bots.includes(bot)||botMap(bot)!==save.map)return;botSpeak(bot,pick(['MORSE!! RUN 😭','NOPE THAT IS MORSE — GET AWAY','BLACK 000, LONG ARMS — RUNNN','WHY IS MORSE HERE?! 😭','DONT LET MORSE GET CLOSE','GET AWAY FROM MORSE NOW','HE TURNED INTO MORSE?? NOPE IM OUT','THATS ACTUALLY MORSE RUNNN']),true);},i*80);
      });
    }else{
      toast('MORSE FORM • OFF');addChatMessage('SYSTEM','Morse signature lost.','system');playerMorseInvisibleUntil=0;playerMorseGhostUntil=0;playerMorseShell=null;playerMorseFlash=0;
      botsHere().forEach(bot=>{if(bot.fleePlayerUntil>time+100)bot.fleePlayerUntil=time+rand(.4,1.4);bot.fear=Math.min(bot.fear,1.1);delete bot.morseEscapeAt;});
    }
  }
...
function spawnMorse(){
  ...
  morse={x:p.x,y:p.y,vx:0,vy:0,r:28,color:'#000000',visible:true,ghost:true,nextAction:rand(.75,1.5),nextSound:rand(.2,.7),target:null,lastTarget:null,targetUntil:0,ghostUntil:0,invisibleUntil:0,shell:null,beam:null,stalkUntil:0,flash:0};
...
function chooseMorseTarget(){
  ...
  let target;if(pool.length&&chance(.94))target=pick(pool);else target=player;
...
function morseBanPlayer(){
  save.bannedUntil=now()+120000;
  rememberSharedPlayerLine('Morse banned me','morseBan',{type:'morseBan',entity:'morse',summary:'Morse banned you',raw:'Morse actually banned you in-game',t:now(),actual:true});
  persist();showBanScreen();bots=[];morse=null;sfx('kick');
}
function morseKickPlayer(){
  rememberSharedPlayerLine('Morse kicked me','morseKick',{type:'morseKick',entity:'morse',summary:'Morse kicked you',raw:'Morse actually kicked you in-game',t:now(),actual:true});
...
function updatePlayer(dt){
  ...
  const armScale=player.mods.morseForm?2.45:(player.mods.longArms?1.85:1),reach=78*armScale;player.left.x=lerp(player.left.x,-42+lx*reach,.22);player.left.y=lerp(player.left.y,28+ly*reach,.22);player.right.x=lerp(player.right.x,42+rx*reach,.22);player.right.y=lerp(player.right.y,28+ry*reach,.22);
  const autoGhost=player.mods.morseForm&&playerMorseGhostUntil>time,autoInvisible=player.mods.morseForm&&playerMorseInvisibleUntil>time;
  const mx=(lx+rx)/2,my=(ly+ry)/2,mag=clamp(Math.hypot(mx,my),0,1);if(mag>.08){const speed=(player.mods.ghost||autoGhost)?780:(player.mods.morseForm?675:610),accel=speed*(.65+.35*mag);player.vx+=mx*accel*dt;player.vy+=my*accel*dt;player.angle=Math.atan2(my,mx)}
  const pump=Math.abs(lx-rx)+Math.abs(ly-ry);if(pump>.8){player.vx+=Math.cos(player.angle)*(player.mods.morseForm?155:120)*dt;player.vy+=Math.sin(player.angle)*(player.mods.morseForm?155:120)*dt}
  const max=(player.mods.ghost||autoGhost)?555:(player.mods.morseForm?430:385),sp=Math.hypot(player.vx,player.vy);if(sp>max){player.vx=player.vx/sp*max;player.vy=player.vy/sp*max}const drag=Math.pow(.13,dt);player.vx*=drag;player.vy*=drag;player.x+=player.vx*dt;player.y+=player.vy*dt;if(!(player.mods.ghost||autoGhost))collideEntity(player);else{const b=mapBounds();player.x=clamp(player.x,b.x1,b.x2);player.y=clamp(player.y,b.y1,b.y2)}
  ...
  if(player.mods.morseForm){
    playerMorseFlash=Math.max(0,playerMorseFlash-dt*1.9);
    if(playerMorseInvisibleUntil&&time>=playerMorseInvisibleUntil){
      playerMorseInvisibleUntil=0;playerMorseFlash=1;scarySound('scrape',true);morseBeep();
      const witness=pick(botsHere().filter(b=>!b.travel&&time>b.chatUntil));if(witness)botSpeak(witness,pick(['HE CAME BACK 😭 RUN','MORSE JUST REAPPEARED','NOPE HE WAS INVISIBLE','HES BACK GET OUT']),true);
    }
    if(playerMorseGhostUntil&&time>=playerMorseGhostUntil){playerMorseGhostUntil=0;playerMorseShell=null;playerMorseFlash=1;}
    if(time>=playerMorseNextSound){
      playerMorseNextSound=time+rand(.85,2.05);if(chance(.58))morseBeep();else scarySound(pick(['whisper','static','scrape','drone','heartbeat']),true);
      if(chance(.58))addChatMessage('Morse',pick(['... --- ...','.-. ..- -.','..-. --- .-.. .-.. --- .--','-- --- .-. ... .','.-.. --- --- -.-']), 'bot');
    }
    if(time>=playerMorseNextPanic){
      playerMorseNextPanic=time+rand(.42,.78);const nearby=botsHere().filter(b=>Math.hypot(b.x-player.x,b.y-player.y)<1050);
      nearby.forEach(b=>{b.knowsMorse=true;b.morseSeen=true;b.followPlayer=false;b.fear=Math.max(b.fear,2.15);b.fleePlayerUntil=Math.max(b.fleePlayerUntil||0,time+3);b.state='fleePlayer';if(!b.morseEscapeAt)b.morseEscapeAt=time+rand(1.4,4.2);});
      const victim=pick(nearby.filter(b=>time>b.chatUntil));if(victim&&chance(.78))botSpeak(victim,pick(['MORSE IS COMING THIS WAY 😭','RUNNN ITS MORSE','NOPE HES TOO CLOSE','GET AWAY FROM 000','WHY IS MORSE FOLLOWING US?!','DONT STOP MOVING','HES GOT THE LONG ARMS RUN','MORSE IS RIGHT BEHIND US']),true);
    }
    if(time>=playerMorseNextAbility){
      playerMorseNextAbility=time+rand(2.7,4.7);const roll=Math.random(),nearby=botsHere().filter(b=>!b.travel),victim=pick(nearby);
      if(roll<.38){
        playerMorseShell={x:player.x,y:player.y,until:time+2.25};playerMorseGhostUntil=time+2.25;playerMorseFlash=1;scarySound('whisper',true);addChatMessage('Morse','... --. .... --- ... -','bot');
        nearby.forEach(b=>{b.fear=Math.max(b.fear,2.3);b.fleePlayerUntil=time+4;b.state='fleePlayer';});if(victim&&time>victim.chatUntil)botSpeak(victim,pick(['MORSE LEFT HIS BODY 😭','GHOST MONKE RUNNN','WHY IS HIS BODY STILL THERE?!','HE WENT GHOST, GET OUT']),true);
      }else if(roll<.72){
        playerMorseInvisibleUntil=time+rand(.9,1.55);playerMorseFlash=1;scarySound('static',true);addChatMessage('Morse','.. -. ...- .. ... .. -... .-.. .','bot');
        if(victim){const ang=rand(0,TAU),dist=rand(95,165),bd=mapBounds();player.x=clamp(victim.x+Math.cos(ang)*dist,bd.x1,bd.x2);player.y=clamp(victim.y+Math.sin(ang)*dist,bd.y1,bd.y2);player.vx=player.vy=0;if(time>victim.chatUntil)botSpeak(victim,pick(['WHERE DID MORSE GO?! 😭','HE JUST VANISHED RUN','NOPE HES INVISIBLE','I CANT SEE 000 ANYMORE']),true);}
      }else{
        scarySound(pick(['alarm','drone','heartbeat','scrape']),true);setTimeout(()=>morseBeep(),180);playerMorseFlash=1;nearby.forEach(b=>{b.fear=Math.max(b.fear,2.15);b.state='fleePlayer';});
        if(victim&&time>victim.chatUntil)botSpeak(victim,pick(['TURN THAT SOUND OFF 😭','MORSE IS PLAYING THAT MUSIC RUN','I HATE THAT SOUND GET OUT','THE BEEPS AGAIN NOPE']),true);
      }
    }
    if(time>=playerMorseNextPhase){
      playerMorseNextPhase=time+rand(3.2,5.8);player.morsePhaseUntil=time+rand(.28,.6);scarySound(chance(.5)?'static':'whisper',true);playerMorseFlash=1;
    }
  }
...
function nearestThreat(bot){
  ...
  if(samePlayer&&player.mods.morseForm&&!player.mods.invisible)consider(player);
  if(save.mode==='Tag'){
...
function updateBots(dt){
  for(const b of bots){
    ...
    if(player.mods.morseForm&&mapKey===save.map&&playerMorseInvisibleUntil<=time&&!player.mods.invisible){
      const pd=Math.hypot(b.x-player.x,b.y-player.y);b.knowsMorse=true;b.morseSeen=true;b.followPlayer=false;b.followUntil=0;b.fear=Math.max(b.fear,pd<650?2.45:1.75);b.fleePlayerUntil=Math.max(b.fleePlayerUntil||0,time+2.2);b.state='fleePlayer';
      if(!b.morseEscapeAt)b.morseEscapeAt=time+rand(1.4,4.5);
      if(b.morseEscapeAt&&time>=b.morseEscapeAt&&!b.travel&&chance(.72)){
        b.morseEscapeAt=time+rand(5,9);const choices=Object.keys(MAPS).filter(k=>k!==save.map);const dest=pick(choices);botSpeak(b,pick(['IM LEAVING THIS MAP 😭','NOPE IM OUT, MORSE IS HERE','GET ME OUT OF HERE','IM NOT STAYING WITH MORSE']),true);sendBotToMap(b,dest,'escaping player Morse');continue;
      }
      if(pd<700&&time>b.chatUntil&&chance(dt*.72))botSpeak(b,pick(['MORSE IS RIGHT THERE 😭','GET AWAY FROM MORSE','NOPE NOPE NOPE RUN','WHY IS 000 CHASING US','DONT LET MORSE TOUCH YOU','RUN, MORSE IS HERE','HES TOO CLOSE RUNNN','WHY ARE HIS ARMS SO LONG 😭']),true);
    }
    ...
    if(b.think<=0){
      ...
      if(player.mods.morseForm&&mapKey===save.map&&!player.mods.invisible)b.state='fleePlayer';
      else if(b.waitUntil>time)b.state='wait';
    ...
    else if(b.state==='fleePlayer'&&mapKey===save.map){dx=b.x-player.x;dy=b.y-player.y;speed*=player.mods.morseForm?1.42:1.08}
...
function updateMorse(dt){
  ...
  if(morse.nextSound<=0){morse.nextSound=rand(.7,1.9);if(chance(.5))morseBeep();else morseSoundboard();}
  ...
  if(morse.nextAction<=0){
    morse.nextAction=rand(.95,2.05);const victim=chooseMorseTarget(),roll=Math.random();morse.lastTarget=morse.target;morse.target=victim;morse.targetUntil=time+rand(2.2,4.4);
    if(roll<.24)morseKickTarget(victim);
    else if(roll<.48)morseBanTarget(victim);
    else if(roll<.64)morseGhostAbility(victim);
    else if(roll<.78)morseInvisibleAbility(victim);
    else if(roll<.90)morseStalkTarget(victim);
...
function drawMorse(){
  ...
  ctx.globalAlpha=ghosting?.42:.94;ctx.strokeStyle='#030303';ctx.lineWidth=14;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(x-12,y);ctx.lineTo(x-88,y+58);ctx.moveTo(x+12,y);ctx.lineTo(x+88,y+58);ctx.stroke();ctx.fillStyle='#000';ctx.beginPath();ctx.arc(x,y+7,28,0,TAU);ctx.fill();ctx.beginPath();ctx.arc(x,y-19,20,0,TAU);ctx.fill();ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(x-7,y-22,4,0,TAU);ctx.arc(x+7,y-22,4,0,TAU);ctx.fill();ctx.strokeStyle='#df4251';ctx.lineWidth=3;ctx.beginPath();ctx.arc(x,y+7,12,0,TAU);ctx.moveTo(x-19,y+7);ctx.lineTo(x+19,y+7);ctx.moveTo(x,y-12);ctx.lineTo(x,y+26);ctx.stroke();text('Morse',x,y+70,12,'#d5d5d5');text(ghosting?'GHOST MONKE':'... --- ...',x,y-62,11,ghosting?'#9fd9ff':'#d05d63');ctx.restore();
}
function drawPlayer(){
  const lw=handWorld(player.left),rw=handWorld(player.right),morseForm=player.mods.morseForm;
  const autoGhost=morseForm&&playerMorseGhostUntil>time,autoInvisible=morseForm&&playerMorseInvisibleUntil>time,morsePhase=morseForm&&player.morsePhaseUntil>time;
  if(morseForm){
    if(playerMorseShell){
      ctx.save();ctx.globalAlpha=.28;ctx.strokeStyle='#050505';ctx.lineWidth=17;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(playerMorseShell.x-12,playerMorseShell.y);ctx.lineTo(playerMorseShell.x-92,playerMorseShell.y+58);ctx.moveTo(playerMorseShell.x+12,playerMorseShell.y);ctx.lineTo(playerMorseShell.x+92,playerMorseShell.y+58);ctx.stroke();ctx.fillStyle='#000';ctx.beginPath();ctx.arc(playerMorseShell.x,playerMorseShell.y+7,29,0,TAU);ctx.fill();ctx.beginPath();ctx.arc(playerMorseShell.x,playerMorseShell.y-20,21,0,TAU);ctx.fill();text('BODY?',playerMorseShell.x,playerMorseShell.y+66,10,'#777');ctx.restore();
    }
    const alpha=player.mods.invisible?.03:autoInvisible?.035:(player.mods.ghost||autoGhost)?(.28+Math.sin(time*12)*.05):morsePhase?.16:1;
    ctx.save();ctx.globalAlpha=alpha;
    if(playerMorseFlash>0){ctx.strokeStyle=`rgba(255,45,58,${.12+.32*playerMorseFlash})`;ctx.lineWidth=36;ctx.beginPath();ctx.arc(player.x,player.y,58+Math.sin(time*12)*7,0,TAU);ctx.stroke();}
    // Long black Morse arms using the player's actual hand positions.
    ctx.strokeStyle='#030303';ctx.lineWidth=18;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(player.x-13,player.y);ctx.lineTo(lw.x,lw.y);ctx.moveTo(player.x+13,player.y);ctx.lineTo(rw.x,rw.y);ctx.stroke();
    ctx.fillStyle='#000';for(const h of [lw,rw]){ctx.beginPath();ctx.arc(h.x,h.y,13,0,TAU);ctx.fill();}
    ctx.beginPath();ctx.arc(player.x,player.y+8,30,0,TAU);ctx.fill();ctx.beginPath();ctx.arc(player.x,player.y-20,21,0,TAU);ctx.fill();
    ctx.fillStyle='#fff';ctx.beginPath();ctx.arc(player.x-7,player.y-23,4.3,0,TAU);ctx.arc(player.x+7,player.y-23,4.3,0,TAU);ctx.fill();
    // 000 targeting mark on the chest so the form matches Morse instead of a normal monke.
    ctx.strokeStyle='#df4251';ctx.lineWidth=3;ctx.beginPath();ctx.arc(player.x,player.y+8,12,0,TAU);ctx.moveTo(player.x-19,player.y+8);ctx.lineTo(player.x+19,player.y+8);ctx.moveTo(player.x,player.y-11);ctx.lineTo(player.x,player.y+27);ctx.stroke();
    if(player.mods.kickGun||player.mods.banGun){ctx.strokeStyle=player.mods.banGun?'rgba(190,100,255,.95)':'rgba(255,80,80,.92)';ctx.lineWidth=4;ctx.setLineDash([10,7]);ctx.beginPath();ctx.moveTo(rw.x,rw.y);ctx.lineTo(rw.x+Math.cos(player.angle)*760,rw.y+Math.sin(player.angle)*760);ctx.stroke();ctx.setLineDash([]);text(player.mods.banGun?'BAN GUN':'KICK GUN',rw.x+Math.cos(player.angle)*110,rw.y+Math.sin(player.angle)*110-14,10,ctx.strokeStyle);}
    if(chance(.28)){ctx.strokeStyle='rgba(255,255,255,.16)';ctx.lineWidth=1;for(let i=0;i<3;i++){ctx.beginPath();ctx.moveTo(player.x-rand(45,85),player.y-rand(20,70));ctx.lineTo(player.x+rand(45,85),player.y-rand(20,70));ctx.stroke();}}
    text('Morse',player.x,player.y+72,13,'#e2e2e2');text('000  •  ... --- ...',player.x,player.y-70,11,'#e05a67');
    if(autoGhost||player.mods.ghost)text('GHOST MONKE',player.x,player.y+88,10,'#b7eaff');
    if(autoInvisible||player.mods.invisible)text('INVISIBLE',player.x,player.y+101,10,'#e8e8ff');
    if(player.chat&&player.chatUntil>time&&$('#chatPanel').classList.contains('hidden')){const w=clamp(player.chat.length*6+18,80,300);ctx.fillStyle='rgba(5,5,5,.92)';roundedRect(player.x-w/2,player.y-118,w,38,10);ctx.fill();text(player.chat,player.x,player.y-99,10,'#fff')}
    ctx.restore();return;
  }
  const alpha=player.mods.invisible?.08:player.mods.ghost?.34:1;ctx.save();ctx.globalAlpha=alpha;
  ...
  ctx.fillStyle=player.color;ctx.beginPath();ctx.arc(player.x,player.y+8,28,0,TAU);ctx.fill();ctx.beginPath();ctx.arc(player.x,player.y-18,20,0,TAU);ctx.fill();for(const h of [lw,rw]){ctx.beginPath();ctx.arc(h.x,h.y,12,0,TAU);ctx.fill()}
  ...
  drawOutfit(player.x,player.y,save.equipped,1.08);text(save.playerName,player.x,player.y+63,12,'#fff');
  ...
  if(player.mods.ghost)text('GHOST',player.x,player.y+76,10,'#b7eaff');if(player.mods.invisible)text('INVISIBLE',player.x,player.y+89,10,'#e8e8ff');
...
function drawVignette(){
  ...
  if(player.mods.morseForm){
    const pulse=(Math.sin(time*8.5)+1)/2,vanish=playerMorseInvisibleUntil>time?1:0;ctx.fillStyle=`rgba(42,0,0,${.035+pulse*.04+(playerMorseFlash||0)*.09+vanish*.035})`;ctx.fillRect(0,0,W,H);
    if(chance(.42)){ctx.fillStyle=`rgba(255,255,255,${.012+(playerMorseFlash||0)*.018})`;for(let i=0;i<6;i++)ctx.fillRect(rand(0,W),rand(0,H),rand(35,300),chance(.15)?2:1);}
  }
}
...
window.__canopyTest={setMap,spawnMorse:()=>{save.room='666';setMap('cave');morseRollDone=true;spawnMorse();persist();},join666:()=>{save.room='666';morse=null;morseRollDone=false;persist();bots=[];fillBots();setMap('cave');resetMode();},say:playerSpeak,openChat:()=>{$('#chatPanel').classList.remove('hidden');},openShop:()=>{$('#panel').classList.remove('hidden');$('.tab[data-tab="shop"]').click();},openSettings:()=>{$('#panel').classList.remove('hidden');$('.tab[data-tab="settings"]').click();},morseKickBot:()=>{if(!morse)spawnMorse();const b=pick(botsHere('cave'));if(b)morseKickTarget(b);},morseBanBot:()=>{if(!morse)spawnMorse();const b=pick(botsHere('cave'));if(b)morseBanTarget(b);},morseGhost:()=>{if(!morse)spawnMorse();morseGhostAbility(chooseMorseTarget());},morseInvisible:()=>{if(!morse)spawnMorse();morseInvisibleAbility(chooseMorseTarget());},morseForm:()=>{player.mods.morseForm=true;playerMorseNextSound=time+.05;playerMorseNextPanic=time+.05;playerMorseNextPhase=time+1;playerMorseNextAbility=time+1.4;playerMorseFlash=1;syncModButtons();morseBeep();scarySound('drone',true);botsHere().forEach(b=>{b.knowsMorse=true;b.morseSeen=true;b.fear=2.45;b.fleePlayerUntil=time+999;b.state='fleePlayer';b.morseEscapeAt=time+rand(1.6,4);});}};
